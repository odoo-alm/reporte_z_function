# =============================================================================
# sync_facturas.py  v4 — usa utils.py
# =============================================================================
...
def _parse_factura(block, filename, msg_id, etiqueta):
    num_documento = _search(r"FACTURA:\s+(\d+)", block)
    ...
    try:
        ...
        return None
    bi_m = re.search(r"BI\s+G(\d+,\d+)%\s+Bs\s*([\-\d.,]+)\s+IVA\s+G\d+,\d+%\s+Bs\s*([\-\d.,]+)", block)
    return {
        "num_documento":      num_documento,
        "tipo_documento":     "FACTURA",
        "num_factura_origen": None,
        "serial_control":     None,
...
def parse_facturas(content, filename, msg_id, etiqueta):
    bloques = [b for b in re.split(r"(?=\s+SENIAT\s+)", content)
               if b.strip() and re.search(r"FACTURA:", b)]
    records = []
    for bloque in bloques:
        r = _parse_factura(bloque, filename, msg_id, etiqueta)
        if r:
            records.append(r)
    logger.info(f"[sync_facturas] {filename}: {len(records)} facturas.")
    return records
...
def _load_to_bq(records, bq_client):
    ...
    job = bq_client.load_table_from_file(
        buffer, f"{BQ_PROJECT}.{BQ_TABLE_FACT}",
    ...
    job.result()
    logger.info(f"[sync_facturas] {job.output_rows} filas cargadas.")
    return job.output_rows

def sync_facturas(service, bq_client, etiquetas, label_map=None, processed_label_id=None):
    ...
    for etiqueta in etiquetas:
        ...
        logger.info(f"[sync_facturas][{etiqueta}] {len(messages)} correos nuevos.")
        ...
        for msg_meta in messages:
            ...
            msg_id = msg_meta["id"]
            try:
                atts = get_attachments(service, msg_id, prefix="FAC", extensions=(".txt",))
                for att in atts:
                    try:
                        records = parse_facturas(att["content"], att["filename"], att["msg_id"], etiqueta)
                        todos.extend(records)
                        stats["procesados"] += len(records)
                    except Exception as e:
                        stats["errores"].append(f"{att['filename']}: {e}")

                # Marcar correo como procesado solo si tuvo adjuntos relevantes
    ...
    logger.info(f"[sync_facturas] procesados={stats['procesados']} cargados={stats['cargados']} errores={len(stats['errores'])}")
    return stats